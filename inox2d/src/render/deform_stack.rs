...
impl DeformStack {
	...
	pub(crate) fn combine(&self, nodes: &InoxNodeTree, node_comps: &World, result: &mut [Vec2]) {
		...
		// I will try to make it work and look as straightforward as possible.

		let mut maybe_meshgroup_uuid: Option<&crate::node::InoxNodeUuid> = None;
		let mut maybe_meshgroup_deform: Option<&Vec<Vec2>> = None;
		let mut maybe_node_id: Option<&crate::node::InoxNodeUuid> = None;

		// Apply direct deform first
		let direct_deforms = self.stack.iter().filter_map(|(deform_source, enabled_deform)| {
			if enabled_deform.0 {
				match (&deform_source, &enabled_deform.1) {
					...
					_ => panic!("Unexpected DeformSource and Deform tuple"),
				}
			} else {
				None
			}
		...
		{
			// Apply transform to meshgroup's space before testing (fast)
			...
			let child_meshgroup_verts =
				child_direct_verts.map(|point| to_meshgroup_space.transform_point3(point.extend(0.0)).truncate());
			...
			// Triangle test (takes most time)
			// let triangle_by_point = meshgroup_mesh.test(child_meshgroup_verts.iter());
			let meshgroup_mesh = node_comps.get::<Mesh>(*meshgroup_uuid).unwrap();
			let mesh_mask = MeshBitMask::new(meshgroup_mesh);
			// TODO: parallel iteration?
			child_meshgroup_verts.enumerate().for_each(|(idx, point)| {
				if let Some(id) = mesh_mask.test(&point) {
			...
			// Calculate deform by meshgroup (fast)
			for (tri_idx, (indices, points_in_tri)) in grouped_points {
				...
				let deform_by_triangle =
					deform_by_parent_triangle(&decompose_matrix, tri[0], &parent_deforms, points_in_tri.iter());

				for (idx, deform_point) in indices.iter().zip(deform_by_triangle) {
					deform_results[*idx] = deform_point;
				}
			}

			// Apply deform by meshgroup (fast)
